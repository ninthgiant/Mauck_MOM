##############################
#
#    MOM_GUI_v11_Main.py - RAM, Aug 19, 2024
#       Builds on v_10 but begins automation process
...
def mom_auto_button(to_do):
    ...
    if 'cal_gradient' in globals(): # do we already have this calculated from a previous bird?
        ...
        if(confirm_continue("Use last bird calibration?")):
            ...
            my_Continue == do_Auto_Calibration(bird_df, 13, 34, 52, 1, 60 )
    ...
    if(my_Continue):
        pass
        # do the auto birdd finding and measuring
        # Do_Multiple_Birds(bird_df, my_Mean_Type) # NEED TO ADD - alert if this isn't true
...
#   
##############
def detect_sections(dataframe, baseline_fraction=0.0002, num_sections=10, window_size=75, std_threshold=500, tolerance=5000):
 
    # Slice the first 5000 lines - make this a parameter
...
def do_Calib_Calc(dataframe, sections_df, mean_baseline, cal1_value, cal2_value, cal3_value):
    ...
    for index, row in sections_df.head(3).iterrows():
        ...
        section_mean = do_Slope_0(dataframe, start_idx, stop_idx, 60, 0, 0.7)
    ...
    return cal_gradient, cal_intercept, cal_r_squared
...
def do_Auto_Calibration(my_dataframe, cal1_value, cal2_value, cal3_value, win_size, the_SPS = 60):
    ...
    if(TRUE):
        ...
        print(sections_df)
                    ...
                    ## Could pass these as an argument...
        cal1_value = float(my_entries2[0].get())
        cal2_value = float(my_entries2[1].get())
        cal3_value = float(my_entries2[2].get())

        cal_gradient, cal_intercept, cal_r_squared = do_Calib_Calc(my_dataframe, sections_df, baseline_cal_mean, cal1_value, cal2_value, cal3_value)
        ...
        if(FALSE):
            ...
            # Print the returned values
            print(f"Baseline Mean Value: {baseline_cal_mean}")
            print(f"Calibration 1 Value: {cal1_mean}")
            print(f"Calibration 2 Value: {cal2_mean}")
            print(f"Calibration 3 Value: {cal3_mean}")
    
    else:  

        #int window_size = int(win_size * the_SPS)   # how many points to we use for initial step across the first minutes
        ...
        # Get Basline defined --- markers = baseline_cal_markers
        # get the baseline_cal_mean

        # Add calibrations as 3 separate pairs of points
            # get cal1_mean, cal1_markers, cal1_Good, axesLimits = getTracePointPair(data, "Cal1[{}]".format(cal1_value), markers, axesLimits)
            # markers = pd.concat([markers, cal1_markers])
