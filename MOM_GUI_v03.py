...
def generate_final_series(input_array, threshold=5, myLen=4):
    ...
    for i in range(len(Count_FWD_array)):
        if sum_array[i] > 0 and inside == 0:  # we have the start
            counter += 1
    ...
    # Count
    count = np.count_nonzero(final_array)

    # Range
    range_value = np.max(final_array) - np.min(final_array)

    # Standard Deviation (STD)
    std_value = np.std(final_array)
